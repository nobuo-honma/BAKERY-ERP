...
import { Package, Wheat, Box, Boxes, ClipboardEdit, ArrowRight, Save, Loader2, AlertCircle, CheckCircle2, ListChecks, TrendingUp, Filter, Lock, Printer, ArrowLeft, Plus } from "lucide-react";
...
export default function InventoryPage() {
  ...
  const fetchInventory = useCallback(async () => {
    ...
    if (itemsData) {
      const formattedItems = itemsData.map((item: any) => {
        ...
        return { id: item.id, name: item.name, item_type: item.item_type, unit: item.unit, unit_size: item.unit_size || 1, unit_price: item.unit_price || 0, safety_stock: item.safety_stock, current_qty: qty };
      });
  ...
  const handleAddNewStock = async () => {
    ...
    try {
      ...
      // 総個数で保存
      const totalPcs = displayToPcs(newStockData.cs, newStockData.p, unitPerCs);
      ...
      if (existingStock) {
        const { cs: existCs, p: existP } = pcsToDisplay(existingStock.total_pieces, unitPerCs);
        if (!confirm(`Lot番号「${newStockData.lotCode}」は既に存在します。\n現在の在庫(${existCs}c/s ${existP}p)に、入力した数を追加加算しますか？`)) {
          setIsProcessing(false);
          return;
        }
        const newTotal = existingStock.total_pieces + totalPcs;
        await supabase.from('product_stocks').update({ total_pieces: newTotal }).eq('id', existingStock.id);
        await supabase.from('inventory_adjustments').insert({ product_id: newStockData.productId, lot_code: newStockData.lotCode, before_qty: existingStock.total_pieces, after_qty: newTotal, reason: `既存Lotへの追加登録` });
      } else {
        await supabase.from('product_stocks').insert({ lot_code: newStockData.lotCode, product_id: newStockData.productId, total_pieces: totalPcs, expiry_date: newStockData.expiryDate });
        await supabase.from('inventory_adjustments').insert({ product_id: newStockData.productId, lot_code: newStockData.lotCode, before_qty: 0, after_qty: totalPcs, reason: `システム導入前在庫の新規登録` });
      }
  ...
  // ============================================================
  // 製品在庫タブのバッチ入力（一括棚卸）
  // batchInputs には cs入力用と p入力用を別々に管理するか、
  // ここでは total_pieces（総個数）を直接 batchInputs に保持する
  // cs/p入力 → displayToPcs → batchInputs[id] に保存
  // ============================================================
  ...
  if (viewMode === 'print') {
    ...
    const printItems = [
      ...
      ...productStocks.map(p => {
        ...
        // 総個数 → cs, p 変換
        const { cs, p: pNum } = pcsToDisplay(p.total_pieces, u);
        return {
          id: p.lot_code,
          category: '製品 (Lot別)',
          name: `${p.products.name} (${p.products.variant_name})`,
          qty: `${cs} c/s${pNum > 0 ? ` ${pNum} p` : ''}`,
        ...
        <div className="flex flex-col mb-4 md:mb-6 gap-3">
          <div className="overflow-x-auto pb-1 -mx-4 px-4 md:mx-0 md:px-0 hide-scrollbar">
            <TabsList className="bg-slate-200/80 flex w-max h-auto p-1.5 rounded-xl">
              <TabsTrigger value="product" className="font-bold py-2.5 px-4 md:px-6 text-sm rounded-lg data-[state=active]:bg-white data-[state=active]:text-blue-700 data-[state=active]:shadow-sm"><Boxes className="w-4 h-4 mr-1.5" /> 製品(Lot別)</TabsTrigger>
              <TabsTrigger value="raw" className="font-bold py-2.5 px-4 md:px-6 text-sm rounded-lg data-[state=active]:bg-white data-[state=active]:text-blue-700 data-[state=active]:shadow-sm"><Wheat className="w-4 h-4 mr-1.5" /> 原材料</TabsTrigger>
              <TabsTrigger value="material" className="font-bold py-2.5 px-4 md:px-6 text-sm rounded-lg data-[state=active]:bg-white data-[state=active]:text-blue-700 data-[state=active]:shadow-sm"><Box className="w-4 h-4 mr-1.5" /> 資材</TabsTrigger>
              <TabsTrigger value="forecast" className="font-bold py-2.5 px-4 md:px-6 text-sm rounded-lg data-[state=active]:bg-white data-[state=active]:text-blue-700 data-[state=active]:shadow-sm bg-blue-100 text-blue-800 ml-2"><TrendingUp className="w-4 h-4 mr-1.5" /> 在庫予測</TabsTrigger>
              <TabsTrigger value="history" className="font-bold py-2.5 px-4 md:px-6 text-sm rounded-lg data-[state=active]:bg-white data-[state=active]:text-slate-700 data-[state=active]:shadow-sm ml-auto"><ClipboardEdit className="w-4 h-4 mr-1.5" /> 履歴</TabsTrigger>
            </TabsList>
          </div>
            ...
            <Table className="min-w-[1000px]">
              ...
              <TableBody>
                {productStocks.map((stock) => {
                  ...
                  // 表示用: batchMode中は batchInputs から、そうでなければ total_pieces から
                  const displayTotal = isBatchMode && batchInputs[stock.id] !== undefined
                    ? Number(batchInputs[stock.id])
                    : stock.total_pieces;

                  const { cs, p } = pcsToDisplay(displayTotal, unit_per_cs);
                      ...
                      <TableCell className="text-right">
                        {isBatchMode ? (
                          <div className="flex items-center justify-end gap-1">
                            ...
                            <Input
                              type="number" inputMode="numeric" min="0" value={cs}
                              onChange={e => {
                                const newCs = Number(e.target.value === "" ? 0 : e.target.value);
                                // cs変更: totalPcs = (newCs * unit_per_cs) + (p * 2)
                                setBatchInputs({ ...batchInputs, [stock.id]: displayToPcs(newCs, p, unit_per_cs) });
                              }}
                              className={`w-20 text-right font-bold h-9 ${isChanged ? 'border-amber-400 bg-white ring-2 ring-amber-200' : 'border-blue-300'}`}
                            />
                        ...
                        {isBatchMode ? (
                          <div className="flex items-center justify-end gap-1">
                            <Input
                              ...
                              max={Math.floor((unit_per_cs - 1) / 2)}
                              value={p}
                              onChange={e => {
                                const newP = Number(e.target.value === "" ? 0 : e.target.value);
                                // p変更: totalPcs = (cs * unit_per_cs) + (newP * 2)
                                setBatchInputs({ ...batchInputs, [stock.id]: displayToPcs(cs, newP, unit_per_cs) });
                              }}
                              className={`w-16 text-right font-bold h-9 ${isChanged ? 'border-amber-400 bg-white ring-2 ring-amber-200' : 'border-blue-300'}`}
                            />
                            <span className="text-xs text-slate-500 font-bold">p</span>
                          </div>
                        ) : <span className="font-bold text-lg text-slate-600">{p} <span className="text-xs font-normal text-slate-400">p</span></span>}
                      </TableCell>
                      <TableCell className="text-center pr-4">
                  ...
                  {isBatchMode ? (
                    <div className="bg-white p-3 rounded-lg border shadow-inner flex flex-col gap-2">
                      <div className="flex justify-between items-center text-sm font-bold text-slate-600">
                        <span>実数入力</span>
                        {isChanged && <span className="text-xs text-amber-700 bg-amber-100 px-2 py-0.5 rounded font-black">変更あり</span>}
                      </div>
                      <div className="flex items-center gap-3">
                        <Input
                          ...
                          className={`flex-1 text-right font-black text-2xl h-14 ${isChanged ? 'border-amber-400 bg-amber-50 focus-visible:ring-amber-500' : 'border-blue-300'}`}
                        />
                    ...
                    <div className="flex justify-between items-end mt-2 pt-2 border-t">
                      <div className="font-black text-3xl text-blue-900">{cs} <span className="text-sm font-normal text-slate-500">c/s</span> <span className="text-xl text-slate-700 ml-1">{p}</span><span className="text-xs font-normal text-slate-400">p</span></div>
        ...
        <TabsContent value="forecast" className="mt-0">
          <div className="bg-white border rounded-lg shadow-sm">
            ...
            <div className="overflow-x-auto max-h-[60vh]">
              <Table className="min-w-max border-collapse">
                <TableHeader className="bg-slate-100 sticky top-0 z-20 shadow-sm">
                  <TableRow>
                    <TableHead className="sticky left-0 bg-slate-100 border-r z-30 w-48 shadow-[2px_0_5px_rgba(0,0,0,0.05)] text-slate-700 font-bold">品目名</TableHead>
                    <TableHead className="w-24 text-right bg-slate-100 border-r z-20 text-slate-700 font-bold">現在庫</TableHead>
                    {forecastResult.dates.map(date => {
                      const d = new Date(date);
                      const isWeekend = d.getDay() === 0 || d.getDay() === 6;
                      return (<TableHead key={date} className={`text-center min-w-[80px] border-r px-2 py-1.5 leading-tight ${isWeekend ? 'text-red-600 bg-red-50/50' : 'text-slate-700'}`}><div className="font-bold">{d.getMonth() + 1}/{d.getDate()}</div><div className="text-[10px] font-normal">{['日', '月', '火', '水', '木', '金', '土'][d.getDay()]}</div></TableHead>);
                    })}
                  </TableRow>
                </TableHeader>
                <TableBody>
            ...
            <div className="p-4 bg-slate-50 border-t flex flex-wrap gap-4 text-xs text-slate-600">
              ...
              <div className="flex items-center gap-1.5"><span className="text-blue-600 font-bold">+数値</span>入荷予定</div>
              <div className="flex items-center gap-1.5"><span className="text-red-500 font-bold">-数値</span>消費予定</div>
            </div>
          </div>
        </TabsContent>
        ...
        <TabsContent value="history" className="mt-0">
          <div className="bg-white border rounded-lg overflow-x-auto shadow-sm pb-10">
            <Table className="min-w-[600px] text-sm">
              <TableHeader className="bg-slate-50">
                <TableRow>
                  <TableHead className="pl-4 w-32">日時</TableHead>
                  <TableHead>対象</TableHead>
                  <TableHead className="text-right">前</TableHead>
                  <TableHead></TableHead>
                  <TableHead className="text-right">後</TableHead>
                  <TableHead className="text-right">差異</TableHead>
                  <TableHead className="pr-4">理由</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {histories.map((hist) => {
        ...
        <DialogContent className="max-w-md bg-white p-6 rounded-xl">
          ...
          <div className="space-y-4 mt-2">
            ...
            <div className="bg-slate-50 p-4 rounded-lg border">
              ...
              {newStockData.productId && (newStockData.cs > 0 || newStockData.p > 0) && (
                <div className="text-xs text-slate-500 mt-2 text-right">
                  保存される総個数: <span className="font-bold text-blue-700">
                    {displayToPcs(newStockData.cs, newStockData.p, productsList.find(p => p.id === newStockData.productId)?.unit || 24)} 個
                  </span>
                </div>
              )}
            </div>
      ...
      <Dialog open={adjustmentModal.isOpen} onOpenChange={(open) => !open && setAdjustmentModal({ ...adjustmentModal, isOpen: false })}>
        <DialogContent className="w-[95vw] max-w-md bg-white p-4 md:p-6 rounded-2xl">
          ...
          <div className="space-y-6 mt-2">
            <div className="bg-slate-50 p-3 md:p-4 rounded-lg border text-center">
              <div className="text-xs font-bold text-slate-500 mb-1">対象品目</div>
              <div className="text-base md:text-lg font-bold text-blue-900 leading-tight">{adjustmentModal.targetName}</div>
              <div className="text-xs text-slate-500 mt-1">
                現在値: {adjustmentModal.currentQty.toLocaleString()} {adjustmentModal.unit}
                {adjustmentModal.type === 'product' && (() => {
                  // 製品の場合、現在値を cs/p で表示
                  const unit_per_cs = productStocks.find(s => s.id === adjustmentModal.targetId)?.products.unit_per_cs || 24;
                  const { cs, p } = pcsToDisplay(adjustmentModal.currentQty, unit_per_cs);
                  return <span className="ml-2 text-blue-700 font-bold">({cs} c/s {p} p)</span>;
                })()}
              </div>
            </div>
            <div className="flex items-center justify-between px-2">
              <div className="text-center">
                <div className="text-[10px] md:text-xs font-bold text-slate-500 mb-1">システム在庫</div>
                <div className="text-xl md:text-2xl font-black text-slate-700">{adjustmentModal.currentQty.toLocaleString()} <span className="text-xs font-normal text-slate-500">{adjustmentModal.unit}</span></div>
              </div>
              <ArrowRight className="w-6 h-6 md:w-8 md:h-8 text-slate-300 mx-1" />
              <div className="text-center">
                <div className="text-[10px] md:text-xs font-bold text-blue-600 mb-1">実際の数 (入力)</div>
                <div className="flex items-end gap-1">
                  <Input type="number" inputMode="decimal" min="0" autoFocus value={actualQty} onChange={e => setActualQty(e.target.value === "" ? "" : Number(e.target.value))} className="w-20 md:w-24 h-12 text-2xl font-bold text-right border-blue-400 focus-visible:ring-blue-500" />
                  <span className="text-[10px] md:text-xs font-bold text-slate-500 pb-2">{adjustmentModal.unit}</span>
                </div>
              </div>
            </div>
            {actualQty !== "" && (
              <div className={`text-center font-bold p-2 md:p-3 rounded-lg ${Number(actualQty) - adjustmentModal.currentQty === 0 ? "bg-slate-100 text-slate-500" : Number(actualQty) - adjustmentModal.currentQty > 0 ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"}`}>
                差異: {Number(actualQty) - adjustmentModal.currentQty > 0 ? "+" : ""}{(Number(actualQty) - adjustmentModal.currentQty).toLocaleString()} {adjustmentModal.unit}
              </div>
            )}
            <div>
              <label className="block text-xs font-bold text-slate-500 mb-2">調整理由</label>
              <select value={adjReason} onChange={e => setAdjReason(e.target.value)} className="w-full border-2 border-slate-200 rounded-lg p-3 bg-white font-bold text-slate-700">
          ...
          <DialogFooter className="mt-4 md:mt-6 border-t pt-4">
            ...
            <Button onClick={handleAdjustmentSubmit} disabled={isProcessing || actualQty === ""} className="w-full md:w-auto bg-blue-600 hover:bg-blue-700 text-white font-bold h-12 md:h-10 text-lg md:text-base">
              {isProcessing ? <Loader2 className="w-5 h-5 mr-2 animate-spin" /> : <Save className="w-5 h-5 mr-2" />} 確定する
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
