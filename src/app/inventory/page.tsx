    ...
    <>
      <div className="hidden md:block bg-white border rounded-lg overflow-x-auto shadow-sm">
        <Table className="min-w-[800px]">
          <TableHeader className="bg-slate-50"><TableRow>
            <TableHead className="w-24 pl-4">品目ID</TableHead><TableHead>品目名</TableHead><TableHead className="text-right">現在庫 (実数)</TableHead>
      ...
      <div className="block md:hidden space-y-3 pb-24">
        {itemList.map((item) => {
          ...
          const isChanged = isBatchMode && batchInputs[item.id] !== undefined && Number(batchInputs[item.id]) !== item.current_qty;
          return (
            <Card key={item.id} className={`p-4 shadow-sm border-2 ${isChanged ? 'bg-amber-50/50 border-amber-300' : 'border-slate-200'}`}>
        ...
        <TabsContent value="product" className="mt-0">
          ...
          <div className="hidden md:block bg-white border rounded-lg overflow-x-auto shadow-sm">
            <Table className="min-w-[1000px]">
              <TableHeader className="bg-slate-50"><TableRow>
                ...
                <TableHead className="text-right">在庫 (c/s)</TableHead><TableHead className="text-right">端数 (p)</TableHead><TableHead className="w-32 text-center pr-4">アクション</TableHead>
              </TableRow></TableHeader>
              <TableBody>
          ...
          <div className="block md:hidden space-y-3 pb-24">
            {productStocks.map((stock) => {
              ...
              return (
                <Card key={stock.id} className={`p-4 shadow-sm border-2 ${isChanged ? 'bg-amber-50/50 border-amber-300' : isExpired ? 'bg-red-50/50 border-red-200' : 'border-slate-200'}`}>
