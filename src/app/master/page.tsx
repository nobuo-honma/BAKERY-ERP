...
import { Database, Edit2, Loader2, Save } from "lucide-react";
...
// --- インライン編集用コンポーネント ---
function EditableCell<T extends EditableValue>({ 
  value, 
  onSave, 
  type = "text", 
  placeholder = "",
  ...
  const handleSave = () => {
    ...
    if (String(val) !== String(value ?? "")) {
      onSave((type === "number" ? Number(val) : val) as T);
    }
  };
  ...
  return (
    <div
      ...
      className={`cursor-pointer hover:bg-slate-100 p-1.5 -m-1.5 rounded flex items-center group min-h-7 transition-colors w-full ${isRightAligned ? "justify-end" : "justify-between"}`}
      title="クリックして編集"
    >
      {isRightAligned && <Edit2 className="h-3 w-3 text-slate-300 opacity-0 group-hover:opacity-100 transition-opacity mr-2 shrink-0" />}
      <span className={`truncate block ${!value ? "text-slate-300" : ""}`}>{value || placeholder}</span>
      {!isRightAligned && <Edit2 className="h-3 w-3 text-slate-300 opacity-0 group-hover:opacity-100 transition-opacity ml-2 shrink-0" />}
    </div>
  );
}
...
export default function MasterPage() {
  ...
  const handleUpdate = async (table: string, id: string, column: string, newValue: EditableValue) => {
    setSavingMsg("保存中...");
    ...
    if (error) {
      ...
      if (table === "products") setProducts(prev => prev.map(p => p.id === id ? { ...p, [column]: newValue } : p));
      else if (table === "items") setItems(prev => prev.map(i => i.id === id ? { ...i, [column]: newValue } : i));
      else if (table === "bom") setBoms(prev => prev.map(b => b.id === id ? { ...b, [column]: newValue } : b));
      else if (table === "customers") setCustomers(prev => prev.map(c => c.id === id ? { ...c, [column]: newValue } : c));
    }
  ...
  if (loading) {
    return (
      <div className="flex justify-center items-center h-64 text-slate-500">
        <Loader2 className="animate-spin h-8 w-8 mr-2" /> データを読み込み中...
      </div>
    );
  }
  ...
  return (
    <div className="bg-white p-6 rounded-lg shadow-sm border border-slate-200 overflow-hidden">
      <div className="flex justify-between items-center mb-6">
        ...
        {savingMsg && (
          <div className="flex items-center text-green-600 font-bold bg-green-50 px-3 py-1 rounded-full text-sm shadow-sm">
            <Save className="h-4 w-4 mr-1" /> {savingMsg}
          </div>
        )}
      </div>
                ...
                {boms.map((bom) => (
                  <TableRow key={bom.id} className="hover:bg-slate-50">
                    <TableCell className="truncate pl-4">
                      ...
                      <div className="text-xs text-slate-500 truncate" title={`${bom.products?.name} (${bom.products?.variant_name})`}>
                        {bom.products?.name} ({bom.products?.variant_name})
                      </div>
                    </TableCell>
        ...
        <TabsContent value="customers">
          <div className="border rounded-md overflow-x-auto shadow-sm">
            <Table className="w-full table-fixed min-w-300">
              <TableHeader className="bg-slate-50">
                <TableRow>
                  <TableHead className="w-25 pl-4">出荷先ID</TableHead>
              ...
              <TableBody>
                {customers.map((customer) => (
                  <TableRow key={customer.id} className="hover:bg-slate-50">
                    ...
                    <TableCell><EditableCell value={customer.notes} placeholder="- 空王 -" onSave={(val) => handleUpdate("customers", customer.id, "notes", val)} /></TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </Table>
