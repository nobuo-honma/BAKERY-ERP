  ...
  const handleSaveShipment = async () => {
    ...
    try {
      ...
      for (const order of selectedGroup.items) {
        ...
        const unitPerCs = order.products?.unit_per_cs || 24; // 1ケースあたりの「個数」
        let totalShippedForThisOrder = 0; // 個数で管理

        for (const stock of stocksForProduct) {
          const input = shipInputs[stock.id];
          const inputCs = Number(input?.cs) || 0; 
          const inputP = Number(input?.p) || 0; // ★パック数
          
          // ★変更: 出荷する「総個数」の計算 (1パック=2個 とする)
          ...
          chunkedOrders.map((chunk, pageIdx) => (
            <div key={pageIdx} className={`w-[210mm] min-h-[297mm] bg-white p-10 print:p-0 shadow-xl print:shadow-none text-black font-sans box-border flex flex-col justify-between gap-8 ${pageIdx < chunkedOrders.length - 1 ? 'page-break mb-8 print:mb-0' : ''}`}>
              {chunk.map((group, gIdx) => {
                ...
                const totalP = group.reduce((sum, s) => sum + s.qty_piece, 0); // p=パック
              ...
              <div className="p-0 flex flex-col h-[calc(100vh-180px)]">
                ...
                <div className="overflow-y-auto flex-1 bg-slate-100/50 p-4 space-y-6">
                  {selectedGroup.items.map(order => {
                    ...
                    // この製品の入力された合計出荷数 (c/s, p)
                    let totalInputCs = 0;
                    let totalInputP = 0;
                    Object.keys(shipInputs).forEach(stockId => {
                      if (productStocks.find(s => s.id === stockId)) {
                        totalInputCs += Number(shipInputs[stockId]?.cs) || 0;
                        totalInputP += Number(shipInputs[stockId]?.p) || 0;
                      }
                    });
                      ...
                      <div key={order.id} className="bg-white border rounded-lg shadow-sm overflow-hidden">
                        <div className="bg-blue-50 border-b border-blue-100 p-3 flex justify-between items-center">
                          ...
                          <div className="flex items-center gap-4">
                            ...
                            <div className={`text-xs font-bold px-2 py-1 rounded ${totalInputCs === order.quantity ? 'bg-green-100 text-green-700' : (totalInputCs > 0 || totalInputP > 0) ? 'bg-amber-100 text-amber-700' : 'bg-slate-200 text-slate-500'}`}>
                              入力計: {totalInputCs} c/s {totalInputP > 0 && `${totalInputP} p`}
                            </div>
                          </div>
                        </div>
                        ...
                        <Table className="text-sm">
                          ...
                          <TableBody>
                            {productStocks.map(stock => {
                              ...
                              const stockPiece = Math.floor((stock.total_pieces % unitPerCs) / 2); // ★残りの個数をパック数に変換
                    ...
                    <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
                      ...
                      <Button 
                        ...
                        disabled={isProcessing || Object.values(shipInputs).every(i => !i.cs && !i.p) || Object.keys(shipInputs).some(stockId => {
                          ...
                          const inputTotalPieces = ((Number(input.cs) || 0) * 24) + ((Number(input.p) || 0) * 2); // 1cs=24個と仮置きしてチェック
