...
export default function OrdersPage() {
  ...
  // ★変更: cs と p に空文字 ("") を許容するように型を修正（入力しやすさ改善のため）
  ...
  useEffect(() => {
    ...
    const fetchBoms = async () => {
      ...
      // 1. まずBOMと、品目情報(items)を取得
      const { data: bomData } = await supabase.from("bom").select(`
        *,
        items ( id, name, item_type )
      `).in("product_id", productIds);

      if (bomData && bomData.length > 0) {
        // 2. BOMに含まれる item_id のリストを作成
        const itemIds = bomData.map((b: any) => b.item_id);

        // 3. 該当する item_stocks (実在庫数) を確実に別クエリで取得
        const { data: stockData } = await supabase.from("item_stocks")
          .select("item_id, quantity")
          .in("item_id", itemIds);

        // 4. BOMデータに在庫数をマージ（合体）する
        const mergedBoms = bomData.map((b: any) => {
          ...
          return {
            ...
            items: {
              ...b.items,
              item_stocks: stock ? [{ quantity: stock.quantity }] : []
            }
          };
        });
  ...
  formData.details.forEach(detail => {
    ...
    const productionPcs = (csVal * selectedProduct.unit_per_cs) + (pVal * 2);
  ...
  return (
    <div className="bg-transparent">
      ...
      <Dialog open={isOpen} onOpenChange={setIsOpen}>
        {/* ★変更: overflow-y-autoを外し、flex-colを指定。全体の高さを固定する */}
                ...
                <div className="bg-blue-50/50 p-4 -mx-2 rounded-md border border-blue-100 space-y-3">
                  ...
                  {formData.details.map((detail, idx) => (
                    <div key={idx} className="flex flex-col sm:flex-row items-end gap-2 bg-white p-2 rounded border shadow-sm relative">
                      ...
                      {/* ★変更: onChangeで空文字を許容し、勝手に0になるのを防ぐ */}
                      ...
                      <div className="w-full sm:w-20"><label className="block text-xs font-bold mb-1 text-slate-500">4. p</label><Input type="number" min="0" value={detail.p !== undefined ? detail.p : ""} onChange={e => updateDetail(idx, 'p', e.target.value === "" ? "" : Number(e.target.value))} className="font-bold text-right h-9" /></div>
