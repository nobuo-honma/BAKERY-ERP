...
export default function OrdersPage() {
  ...
  const openModal = (order?: Order) => {
    if (order) {
      ...
      // ★変更: DBの総個数からc/sとp(パック:2個)を逆算してフォームにセット
  ...
  const handleSaveOrder = async () => {
    if (!formData.customerId || !formData.shipDate || !formData.plannedShipDate) { alert("出荷予定日などの必須項目を入力してください。"); return; }
    ...
    try {
      if (editingOrder) {
        ...
        const selectedProduct = products.find(p => p.id === detail.productId);
        const totalQuantity = (detail.cs * (selectedProduct?.unit_per_cs || 24)) + (detail.p * 2); // ★DBには総個数で保存
