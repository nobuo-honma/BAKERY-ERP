...
export default function OrdersPage() {
  ...
  const [formData, setFormData] = useState<{
    ...
    details: { productId: string; cs: number; p: number; selectedName: string }[];
  ...
  const openModal = (order?: Order) => {
    if (order) {
      ...
      const cs = Math.floor(order.quantity / unitPerCs);
      const p = Math.floor((order.quantity % unitPerCs) / 2);
      ...
      setFormData({
        ...
        details: [{ productId: order.product_id, cs: cs, p: p, selectedName: order.products?.name || "" }]
      });
    } else resetForm();
    setIsOpen(true);
  };
  ...
  const handleSaveOrder = async () => {
    if (!formData.customerId || !formData.shipDate || !formData.plannedShipDate) { alert("出荷予定日と着予定日、出荷先などの必須項目を入力してください。"); return; }
    ...
    try {
      if (editingOrder) {
        ...
        const orderData = { order_date: formData.date, planned_ship_date: formData.plannedShipDate, desired_ship_date: formData.shipDate, customer_id: formData.customerId, customer_order_no: formData.customerOrderNo, product_id: detail.productId, quantity: totalQuantity };
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-4">
            <div className="space-y-4 bg-slate-50 p-6 rounded-lg border shadow-inner">
              <div className="grid grid-cols-3 gap-4">
                ...
                {/* --- ★修正: 出荷先の検索(datalist)を復活・確実動作 --- */}
                <div>
                  <label className="block text-xs font-bold mb-1 text-slate-700">出荷先 <span className="text-red-500">*</span></label>
                  <Input
                    list="customers-list"
                    placeholder="検索..."
                  ...
                  <datalist id="customers-list">
                    {/* ここに出荷先マスタ(customers)のデータがリストアップされ、検索候補に出ます */}
          ...
          return (
            <Card key={order.id} className="hover:shadow-md transition-all border-slate-200 relative group">
              ...
              <CardContent className="pt-4 space-y-2 text-sm text-slate-600">
                ...
                <div className="flex justify-between items-center pt-1"><span>数量:</span><span className="font-black text-xl text-blue-600">{displayCs} <span className="text-sm font-normal text-slate-500">c/s</span> {displayP > 0 && <span className="text-slate-700 ml-1">{displayP} <span className="text-xs font-normal text-slate-500">p</span></span>}</span></div>
