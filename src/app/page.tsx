...
export default function Dashboard() {
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      // ★変更: 出荷予定日が本日のものを取得
      const { data: shipData } = await supabase.from("orders").select("quantity, customers(name)").eq("planned_ship_date", todayStr).neq("status", "shipped");
      ...
      const { data: ordersData } = await supabase.from("orders")
        .select("id, planned_ship_date, desired_ship_date, status, quantity, customer_order_no, customers(name), products(name, variant_name), production_plans(production_date, planned_cs, status)")
      ...
      if (ordersData) {
        const processedOrders = ordersData.map((order: any) => {
          ...
          const isFullyPlanned = plannedCs >= order.quantity;
                  ...
                  <div className="flex flex-col md:flex-row justify-between md:items-center gap-4">
                    ...
                    <div className="shrink-0 md:w-64 md:text-right flex flex-row md:flex-col items-center md:items-end justify-between gap-2 border-l border-slate-200 pl-4">
                      <div>
                        ...
                        {isShipmentWarning && !isLate && <span className="block text-[10px] text-amber-600 mt-1 font-bold flex items-center justify-end gap-0.5"><AlertTriangle className="w-3 h-3" />出荷が間に合いません</span>}
                      </div>
                  ...
                  <div className="bg-slate-50/50 p-3 rounded-lg border border-slate-200 flex flex-col md:flex-row md:items-center gap-4 md:gap-8">
                    <div className="flex-1 w-full">
                      <div className="flex justify-between items-end text-xs mb-1.5 font-bold text-slate-600">
                        ...
                        {order.progressPercent === 100 ? <span className="text-green-600 flex items-center gap-1"><CheckCircle2 className="w-3 h-3" /> 完成済</span> : <span className="text-blue-700">{order.completedCs} / {order.quantity} c/s <span className="text-[10px] font-normal text-slate-500 ml-1">({order.progressPercent}%)</span></span>}
                      </div>
