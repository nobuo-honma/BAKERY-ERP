...
type Order = {
  id: string; planned_ship_date: string; desired_ship_date: string; status: string; quantity: number;
  customer_order_no?: string;
  customers?: { name: string };
  products?: { name: string; variant_name: string; unit_per_cs: number };
  progressPercent?: number; completedPcs?: number; completionDateStr?: string;
  shipAvailableDateStr?: string; shipAvailableDateObj?: Date | null; isFullyPlanned?: boolean;
};

type OrderGroup = {
  groupId: string; customerName: string; customerOrderNo: string;
  plannedShipDate: string; desiredShipDate: string; status: string; items: Order[];
  totalProgress: number; totalCompletedPcs: number; totalQuantityPcs: number;
  totalCompletedCs: number; totalQuantityCs: number;
  isLate: boolean; shipWarning: boolean; completionDateStr: string; shipAvailableDateStr: string;
};
...
export default function Dashboard() {
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      // 本日の製造予定
      const { data: prodData } = await supabase.from("production_plans").select("planned_cs, products(name, unit_per_cs)").eq("production_date", todayStr);
      ...
      if (shipData && shipData.length > 0) {
        const totalCs = shipData.reduce((sum: number, o: any) => {
          const unitPerCs = o.products?.unit_per_cs || 24;
          return sum + Math.floor(o.quantity / unitPerCs);
        }, 0);
      ...
      if (ordersData) {
        ...
        ordersData.forEach((order: any) => {
          ...
          // 完了済み製造分の個数 (planned_cs × unit_per_cs)
          const completedPcs = plans
            .filter((p: any) => p.status === 'completed')
            .reduce((sum: number, p: any) => sum + (p.planned_cs * unitPerCs), 0);
          // 計画済み個数
          const plannedPcs = plans.reduce((sum: number, p: any) => sum + (p.planned_cs * unitPerCs), 0);

          // quantity は総個数
          const progressPercent = order.quantity > 0 ? Math.min(100, Math.floor((completedPcs / order.quantity) * 100)) : 0;
          const isFullyPlanned = plannedPcs >= order.quantity;
          ...
          const { cs: completedCs } = pcsToDisplay(completedPcs, unitPerCs);
          const { cs: totalCs } = pcsToDisplay(order.quantity, unitPerCs);
          ...
          if (!groups[gId]) {
            ...
            groups[gId] = {
              ...
              totalProgress: 0, totalCompletedPcs: 0, totalQuantityPcs: 0,
              totalCompletedCs: 0, totalQuantityCs: 0,
        ...
        Object.values(groups).forEach(g => {
          let totalPcs = 0;
          let totalCompletedPcs = 0;
          let latestCompletionTime = 0;
          let allPlanned = true;

          g.items.forEach((item: any) => {
            const unitPerCs = item.products?.unit_per_cs || 24;
            totalPcs += item.quantity; // 総個数
            totalCompletedPcs += item.completedPcs || 0;
            g.totalQuantityCs += Math.floor(item.quantity / unitPerCs);
          ...
          g.totalQuantityPcs = totalPcs;
          g.totalCompletedPcs = totalCompletedPcs;
          g.totalProgress = totalPcs > 0 ? Math.min(100, Math.floor((totalCompletedPcs / totalPcs) * 100)) : 0;
          ...
          if (g.items.some((i: any) => i.status === 'in_production')) g.status = 'in_production';
        });
  ...
  const shortageText = alerts.shortages.length > 0
    ? `不足: ${alerts.shortages.slice(0, 2).join(", ")}${alerts.shortages.length > 2 ? ' 他' : ''}`
    : "";
  const warningText = alerts.warnings.length > 0
    ? `注意: ${alerts.warnings.slice(0, 2).join(", ")}${alerts.warnings.length > 2 ? ' 他' : ''}`
    : "";
        ...
        <Card className={`${alerts.total > 0 ? 'border-red-200 bg-red-50' : 'border-slate-200 bg-slate-50'} shadow-sm`}>
          ...
          <CardContent>
            ...
            <div className={`text-sm mt-2 font-bold ${alerts.total > 0 ? 'text-red-600' : 'text-slate-400'}`}>
              {alerts.total > 0
                ? <>{shortageText && <div>{shortageText}</div>}{warningText && <div>{warningText}</div>}</>
                : "すべて安全在庫を満たしています"}
            </div>
          </CardContent>
        </Card>
                ...
                <div className="flex flex-col md:flex-row justify-between md:items-start gap-4 border-b pb-3">
                  <div className="flex-1 min-w-0">
                    <div className="text-xs text-slate-500 mb-1 flex items-center gap-2">
                      ...
                      {group.customerOrderNo && (
                        <Badge variant="outline" className="text-[10px] bg-white text-slate-500 py-0">
                          <FileText className="w-3 h-3 mr-1" /> 発注: {group.customerOrderNo}
                        </Badge>
                      )}
                    </div>
                  ...
                  <div className="shrink-0 md:w-56 md:text-right flex flex-row md:flex-col items-center md:items-end justify-between gap-2">
                    <div>
                      <div className={`text-sm font-bold ${group.isLate ? 'text-red-600' : 'text-blue-800'}`}>
                        出荷予定: {new Date(group.plannedShipDate).toLocaleDateString('ja-JP')}
                        {group.isLate && <span className="block text-[10px] text-red-500 font-bold">(期限超過!)</span>}
                      </div>
                      <div className="text-xs text-slate-500 font-bold mt-1">着予定(納期): {new Date(group.desiredShipDate).toLocaleDateString('ja-JP')}</div>
                      {group.shipWarning && !group.isLate && (
                        <span className="flex text-[10px] text-amber-600 mt-1 font-bold items-center justify-end gap-0.5">
                          <AlertTriangle className="w-3 h-3" />出荷が間に合いません
                        </span>
                      )}
                    </div>
                    {group.status === 'in_production'
                      ? <Badge className="bg-blue-100 text-blue-800 border-none px-2 py-0.5 text-xs shadow-sm">製造中あり</Badge>
                      : <Badge className="bg-amber-100 text-amber-800 border-none px-2 py-0.5 text-xs shadow-sm">未処理 (引当待)</Badge>}
                  </div>
                </div>
                    ...
                    return (
                      <div key={item.id} className="bg-white border rounded p-2 text-sm flex justify-between items-center shadow-sm">
                        <div className="font-bold text-slate-700 truncate">
                          {item.products?.name} <span className="text-[10px] font-normal text-slate-500">({item.products?.variant_name})</span>
                        </div>
                        <div className="font-black text-slate-800 shrink-0 ml-2">
                          {cs} <span className="text-[10px] font-normal">c/s</span>
                          {p > 0 && <span className="ml-1">{p} <span className="text-[10px] font-normal">p</span></span>}
                        </div>
                      </div>
                    );
                  })}
                    ...
                    <div className="flex justify-between items-end text-xs mb-1.5 font-bold text-slate-600">
                      ...
                      {group.totalProgress === 100 ? (
                        ...
                        <span className="text-blue-700">
                          {group.totalCompletedCs} / {group.totalQuantityCs} c/s
                          <span className="text-[10px] font-normal text-slate-500 ml-1">({group.totalProgress}%)</span>
                        </span>
                      )}
                    </div>
