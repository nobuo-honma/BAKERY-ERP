...
import { Factory, Calendar as CalendarIcon, PackageCheck, Loader2, ListPlus, CheckCircle2, CalendarDays, Printer, ArrowLeft, ChevronLeft, ChevronRight, Edit, Trash2, Play, CheckCircle, Truck, Flag } from "lucide-react";
...
export default function ProductionPage() {
  ...
  const handleUpdatePlan = async () => {
    ...
    if (!error) { setEditingPlan(null); fetchCalendarPlans(); fetchData(); } else alert("更新失敗: " + error.message);
    setIsProcessing(false);
  };

  const handleDeletePlan = async () => {
    if (!editingPlan) return;
    if (editingPlan.status !== 'planned') { alert("製造開始済みの計画は削除できません。"); return; }
    if (!confirm("この製造計画を削除しますか？")) return;
    setIsProcessing(true);
    const { error } = await supabase.from("production_plans").delete().eq("id", editingPlan.id);
    if (!error) { setEditingPlan(null); fetchCalendarPlans(); fetchData(); } else alert("削除失敗: " + error.message);
    setIsProcessing(false);
  };
  ...
  const openEventDialog = (ev?: Event, dateStr?: string) => {
    if (ev) {
      setEditingEvent(ev); setEventDate(ev.event_date); setEventTitle(ev.title); setEventNotes(ev.notes || "");
    } else {
      setEditingEvent(null); setEventDate(dateStr || new Date().toISOString().split('T')[0]); setEventTitle(""); setEventNotes("");
    }
  ...
  const handleSaveEvent = async () => {
    ...
    setIsProcessing(true);
    const eventData = { event_date: eventDate, title: eventTitle, notes: eventNotes };
    if (editingEvent) {
      ...
      <div className="bg-white min-h-screen print:p-0 print:m-0">
        ...
        <div className="flex justify-between items-center mb-6 print:hidden bg-slate-50 p-4 rounded-lg border shadow-sm">
          ...
          <div className="flex gap-2">
            <Button onClick={() => openEventDialog()} className="bg-slate-700 hover:bg-slate-800 text-white gap-2 font-bold shadow-sm"><Flag className="h-4 w-4" /> イベント登録</Button>
                    ...
                    {day && (
                      <>
                        <div className="flex justify-between items-start mb-1">
                          <div onClick={() => openEventDialog(undefined, dateStr || undefined)} className="text-slate-300 hover:text-blue-500 cursor-pointer print:hidden p-0.5"><PlusIcon /></div>
                        ...
                        <div className="space-y-1.5 print:space-y-1">
                          {/* ① イベント */}
                          {dayEvents.map(ev => (
                            <div key={ev.id} onClick={() => openEventDialog(ev)} className="bg-slate-100 border border-slate-300 rounded p-1.5 print:p-1 print:border-black cursor-pointer hover:bg-slate-200 text-xs leading-tight wrap-break-word relative group">
                          ...
                          {dayPlans.map(plan => {
                            ...
                            return (
                              <div key={plan.id} onClick={() => openEditDialog(plan)} className={`${cardColor} border rounded p-1.5 print:p-1 print:border-black print:bg-white cursor-pointer hover:shadow-md transition-shadow text-xs leading-tight wrap-break-word relative group`}>
                                <div className="absolute top-1 right-1 opacity-0 group-hover:opacity-100 transition-opacity print:hidden text-slate-400"><Edit className="h-3 w-3" /></div>
              ...
              {editingPlan && (
                <div className="space-y-4 mt-2">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div><label className="block text-xs font-bold text-slate-500 mb-1">製造予定日</label><Input type="date" value={editDate} onChange={e => setEditDate(e.target.value)} disabled={editingPlan.status !== 'planned'} className="h-9" /></div>
                    <div><label className="block text-xs font-bold text-slate-500 mb-1">製造量 (kg)</label><Input type="number" value={editKg} onChange={e => setEditKg(e.target.value === "" ? "" : Number(e.target.value))} disabled={editingPlan.status !== 'planned'} className="h-9 text-right" /></div>
                  </div>
                  <div><label className="block text-xs font-bold text-slate-500 mb-1">備考</label><Input value={editNotes} onChange={e => setEditNotes(e.target.value)} className="h-9" placeholder="備考を入力..." /></div>
