...
import { Beaker, Loader2, Save, Lock, Edit, Printer, ArrowLeft } from "lucide-react";
...
export default function KeepSamplesPage() {
    ...
    const handleSaveUsage = async () => {
        ...
        if (uQty > editingSample.saved_quantity) { alert("エラー: 使用数が保存数を超えています！"); return; }
        setIsProcessing(true);
        const updates = { used_quantity: uQty, usage_purpose: uQty > 0 ? editPurpose : null, used_date: uQty > 0 ? editUsedDate : null, updated_at: new Date().toISOString() };
        const { error } = await supabase.from("keep_samples").update(updates).eq("id", editingSample.id);
        if (!error) { setEditingSample(null); fetchSamples(); alert("使用記録を保存しました！"); } else alert("エラー: " + error.message);
        setIsProcessing(false);
    };

    // =======================================================================
    // ★追加: キープサンプル管理記録 (PDFプレビュー・印刷画面)
    // =======================================================================
    if (viewMode === 'print') {
        // 3件ずつページに分割
        const chunkedSamples = [];
        for (let i = 0; i < samples.length; i += 3) {
            chunkedSamples.push(samples.slice(i, i + 3));
        }
                    ...
                    chunkedSamples.map((chunk, pageIdx) => (
                        <div key={pageIdx} className={`w-[297mm] h-[210mm] bg-white pt-8 pb-4 px-12 print:p-0 shadow-xl print:shadow-none text-black font-sans box-border flex flex-col ${pageIdx < chunkedSamples.length - 1 ? 'page-break mb-8 print:mb-0' : ''}`}>
                            ...
                            <div className="flex justify-between items-end mb-4 pr-4">
                                ...
                                <table className="border-collapse border-[2px] border-black text-center text-sm w-48">
                                    <tbody>
                                        <tr><th className="border border-black w-1/2 py-1 font-medium">施設長</th><th className="border border-black w-1/2 py-1 font-medium">担当</th></tr>
                                        <tr><td className="border border-black h-16"></td><td className="border border-black h-16"></td></tr>
                                    </tbody>
                                </table>
                            </div>
                            ...
                            <div className="flex-1 flex flex-col gap-4">
                                {chunk.map(sample => {
                                    // 日付フォーマット
                                    const dP = new Date(sample.production_date);
                                    const pStr = `${dP.getFullYear()}.${String(dP.getMonth() + 1).padStart(2, '0')}.${String(dP.getDate()).padStart(2, '0')}`;
                                    const dE = new Date(sample.expiry_date);
                                    const eStr = `${dE.getFullYear()}.${String(dE.getMonth() + 1).padStart(2, '0')}.${String(dE.getDate()).padStart(2, '0')}`;

                                    return (
                                        <div key={sample.id} className="flex border-[2px] border-black h-[48mm] text-[13px]">
                                            {/* 左ブロック */}
                                            <div className="w-[60mm] flex flex-col border-r border-black shrink-0">
                                                <div className="flex-1 flex border-b border-black">
                                                    <div className="w-[20mm] p-2 font-medium border-r border-black flex items-center justify-center">ロット</div>
                                                    <div className="flex-1 p-2 flex items-center justify-center font-bold text-base tracking-widest">{sample.lot_code}</div>
                                                </div>
                                                <div className="flex-1 flex border-b border-black">
                                                    <div className="w-[20mm] p-2 font-medium border-r border-black flex flex-col items-center justify-center leading-tight"><span>賞味</span><span>期限</span></div>
                                                    <div className="flex-1 p-2 flex items-center justify-center font-bold tracking-widest">{eStr}</div>
                                                </div>
                                                <div className="flex-1 flex">
                                                    <div className="w-[20mm] p-2 font-medium border-r border-black flex flex-col items-center justify-center leading-tight"><span>管理</span><span>番号</span></div>
                                                    <div className="flex-1 p-2 flex items-center justify-center font-bold">{sample.management_no}</div>
                                                </div>
                                            </div>
                                            ...
                                            <div className="w-[65mm] flex flex-col border-r border-black shrink-0">
                                                <div className="flex-1 flex border-b border-black">
                                                    <div className="w-[20mm] p-2 font-medium border-r border-black flex flex-col items-center justify-center leading-tight"><span>製造</span><span>種類</span></div>
                                                    <div className="flex-1 p-2 flex items-center justify-center text-center text-xs font-bold leading-tight">{sample.products?.name} <br />({sample.products?.variant_name})</div>
                                                </div>
                                                <div className="flex-1 flex border-b border-black">
                                                    <div className="w-[20mm] p-2 font-medium border-r border-black flex flex-col items-center justify-center leading-tight"><span>製造</span><span>年月日</span></div>
                                                    <div className="flex-1 p-2 flex items-center justify-center font-bold tracking-widest">{pStr}</div>
                                                </div>
                                                <div className="flex-1 flex">
                                                    <div className="w-[20mm] p-2 font-medium border-r border-black flex items-center justify-center">保存数</div>
                                                    <div className="flex-1 p-2 pr-4 flex items-center justify-end font-bold text-lg">{sample.saved_quantity} <span className="text-[10px] font-normal ml-1">個</span></div>
                                                </div>
                                            </div>

                                            {/* 使用用途 */}
                                            <div className="w-[12mm] border-r border-black flex items-center justify-center font-medium text-xs shrink-0" style={{ writingMode: 'vertical-rl', textOrientation: 'upright', letterSpacing: '0.2em' }}>
                                                使用用途名
                                            </div>
                                                ...
                                                {Array.from({ length: 5 }).map((_, i) => {
                                                    ...
                                                    return (
                                                        <div key={i} className="flex-1 flex border-b border-black last:border-b-0">
                                                            <div className="w-[22mm] border-r border-black flex flex-col items-center justify-center bg-slate-50 print:bg-transparent leading-tight shrink-0">
                                                                <span className="text-[10px]">使用</span><span className="text-[10px]">年月日</span>
                                                            </div>
                                            ...
                                            <div className="w-[12mm] flex flex-col shrink-0">
                                                {Array.from({ length: 5 }).map((_, i) => (
                                                    <div key={i} className="flex-1 border-b border-black last:border-b-0 flex items-center justify-center text-[9px] font-medium text-slate-400 print:text-black/50">印</div>
                                                ))}
                                            </div>
                                        </div>
                                    );
                                ...
                                {Array.from({ length: 3 - chunk.length }).map((_, idx) => (
                                    <div key={`empty-${idx}`} className="flex border-[2px] border-black h-[48mm] text-sm opacity-20">
                                        <div className="w-[60mm] border-r border-black"></div>
                                        <div className="w-[65mm] border-r border-black"></div>
                                        <div className="w-[12mm] border-r border-black"></div>
                                        <div className="flex-1 border-r border-black flex flex-col">{Array.from({ length: 5 }).map((_, i) => <div key={i} className="flex-1 border-b border-black last:border-b-0"></div>)}</div>
                                        <div className="w-[12mm] flex flex-col">{Array.from({ length: 5 }).map((_, i) => <div key={i} className="flex-1 border-b border-black last:border-b-0"></div>)}</div>
                                    </div>
                                ))}
                            </div>
                        </div>
                    ...
                    {editingSample && (
                        <div className="space-y-4 mt-2">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div><label className="block text-xs font-bold text-slate-500 mb-1">使用した数 (個)</label><Input type="number" min="0" max={editingSample.saved_quantity} value={editUsedQty} onChange={e => setEditUsedQty(e.target.value === "" ? "" : Number(e.target.value))} className="text-xl font-bold text-right border-blue-300 h-10" /></div>
                            ...
                            <div className="text-xs text-slate-500 text-center mt-2">※使用数を入力すると、残数は自動的に {editingSample.saved_quantity - (Number(editUsedQty) || 0)} 個になります。</div>
                            <DialogFooter className="mt-4 pt-4 border-t flex gap-2 w-full">
                                ...
                                <Button onClick={handleSaveUsage} disabled={isProcessing || editUsedQty === ""} className="flex-1 bg-blue-600 hover:bg-blue-700 text-white font-bold">{isProcessing ? <Loader2 className="animate-spin h-4 w-4 mr-2" /> : <Save className="h-4 w-4 mr-2" />} 記録を保存</Button>
                            </DialogFooter>
                        </div>
                    )}
                </DialogContent>
