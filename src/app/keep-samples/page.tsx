...
export default function KeepSamplesPage() {
    ...
    const handleSaveUsage = async () => {
        ...
        const updates = { saved_quantity: sQty, used_quantity: uQty, usage_purpose: uQty > 0 ? editPurpose : null, used_date: uQty > 0 ? editUsedDate : null, updated_at: new Date().toISOString() };
        const { error } = await supabase.from("keep_samples").update(updates).eq("id", editingSample.id);
        if (!error) { setEditingSample(null); fetchSamples(); alert("記録を更新しました！"); } else alert("エラー: " + error.message);
    ...
    const handleDeleteSample = async () => {
        ...
        if (!error) { setEditingSample(null); fetchSamples(); } else alert("削除エラー: " + error.message);
        setIsProcessing(false);
    };
                        ...
                        <div key={pageIdx} className={`w-[297mm] h-[210mm] bg-white pt-8 pb-4 px-12 print:p-0 shadow-xl print:shadow-none text-black font-sans box-border flex flex-col ${pageIdx < chunkedSamples.length - 1 ? 'page-break mb-8 print:mb-0' : ''}`}>
                            ...
                            <div className="flex-1 flex flex-col gap-4">
                                ...
                                {Array.from({ length: 3 - chunk.length }).map((_, idx) => (
                                    <div key={`empty-${idx}`} className="flex border-[2px] border-black h-[48mm] text-sm opacity-20"><div className="w-[60mm] border-r border-black"></div><div className="w-[65mm] border-r border-black"></div><div className="w-[12mm] border-r border-black"></div><div className="flex-1 border-r border-black flex flex-col">{Array.from({ length: 5 }).map((_, i) => <div key={i} className="flex-1 border-b border-black last:border-b-0"></div>)}</div><div className="w-[12mm] flex flex-col">{Array.from({ length: 5 }).map((_, i) => <div key={i} className="flex-1 border-b border-black last:border-b-0"></div>)}</div></div>
                                ))}
                            </div>
                        </div>
                    ))
    ...
    return (
        <div className="bg-transparent">
            ...
            <div className="bg-white border rounded-lg shadow-sm overflow-x-auto">
                {/* ★変更: min-wを調整して画面内に収まりやすくしました */}
                <Table className="min-w-[900px] w-full">
                    <TableHeader className="bg-slate-50">
                        <TableRow>
                            <TableHead className="w-10 text-center">No.</TableHead>
                            <TableHead className="w-32 pl-4">管理番号 / Lot</TableHead>
                            <TableHead className="w-40">製品名 / 種類</TableHead>
                            <TableHead className="w-24 text-center">製造日 / 期限</TableHead>
                            <TableHead className="w-16 text-right">保存数</TableHead>
                            <TableHead className="w-16 text-right text-red-600">使用数</TableHead>
                            <TableHead className="w-16 text-right font-bold text-blue-700">残数</TableHead>
                            <TableHead className="w-32">使用用途 / 使用日</TableHead>
                            <TableHead className="w-20 text-center">状態</TableHead>
                            <TableHead className="w-24 text-center pr-4">アクション</TableHead>
                        </TableRow>
                    </TableHeader>
                    <TableBody>
                            ...
                            return (
                                <TableRow key={sample.id} className={isUsedUp ? "bg-slate-50/50 opacity-70" : "hover:bg-slate-50"}>
                                    ...
                                    <TableCell className="truncate max-w-[150px]">
                                        <div className="font-bold text-slate-800 truncate" title={sample.products?.name}>{sample.products?.name}</div>
                                        <div className="text-xs text-slate-500 truncate" title={sample.products?.variant_name}>{sample.products?.variant_name}</div>
                                    </TableCell>
                                    ...
                                    <TableCell className={`text-right font-black text-lg ${isUsedUp ? 'text-slate-400' : 'text-blue-700'}`}>{remainQty} <span className="text-[10px] font-normal">p</span></TableCell>

                                    <TableCell className="text-xs truncate max-w-[150px]">
                                        {sample.used_quantity > 0 ? (
                                            <>
                                                <div className="font-bold text-slate-700 truncate" title={sample.usage_purpose}>{sample.usage_purpose}</div>
                                    ...
                                    <TableCell className="text-center">
                                        {isUsedUp ? <Badge className="bg-slate-200 text-slate-600 border-none shadow-none text-[10px] px-1.5 py-0">使用済(残0)</Badge> :
                                            isPartiallyUsed ? <Badge className="bg-amber-100 text-amber-800 border-none shadow-sm text-[10px] px-1.5 py-0">一部使用</Badge> :
                                                <Badge className="bg-green-100 text-green-800 border-none shadow-sm text-[10px] px-1.5 py-0">保管中</Badge>}
                                    </TableCell>

                                    <TableCell className="text-center pr-4">
                                        {canEdit ? (
                                            <Button variant="outline" size="sm" onClick={() => openEditModal(sample)} className="text-blue-600 border-blue-200 hover:bg-blue-50 px-2 h-7 text-xs">
                                                <Edit className="h-3 w-3 mr-1" /> 詳細
                                            </Button>
                                        ) : (
            ...
            <Dialog open={!!editingSample} onOpenChange={(open) => !open && setEditingSample(null)}>
                <DialogContent className="max-w-sm bg-white p-6 rounded-xl">
                    <DialogHeader><DialogTitle className="flex items-center gap-2 text-slate-800"><Beaker className="h-5 w-5 text-blue-600" /> サンプル記録の編集</DialogTitle></DialogHeader>
                    {editingSample && (
                        <div className="space-y-4 mt-2">
                            ...
                            <div className="grid grid-cols-2 gap-4 border-b pb-4">
                                <div><label className="block text-xs font-bold text-slate-500 mb-1">保存数 (変更可)</label><Input type="number" min="1" value={editSavedQty} onChange={e => setEditSavedQty(e.target.value === "" ? "" : Number(e.target.value))} className="text-lg font-bold text-right border-slate-300 h-10 bg-white" /></div>
                                <div><label className="block text-xs font-bold text-slate-500 mb-1">使用した数</label><Input type="number" min="0" value={editUsedQty} onChange={e => setEditUsedQty(e.target.value === "" ? "" : Number(e.target.value))} className="text-lg font-bold text-right border-blue-300 h-10 bg-white focus-visible:ring-blue-500" /></div>
                            </div>
                            ...
                            <div><label className="block text-xs font-bold text-slate-500 mb-1">使用用途</label><Input value={editPurpose} onChange={e => setEditPurpose(e.target.value)} placeholder="例: 官能検査、菌検査..." className="h-10" /></div>

                            <div className="text-xs font-bold text-blue-700 text-center mt-2 bg-blue-50 p-2 rounded">現在の残数: {(Number(editSavedQty) || 0) - (Number(editUsedQty) || 0)} 個</div>

                            <DialogFooter className="mt-4 pt-4 border-t flex flex-col sm:flex-row gap-2 w-full">
                                <Button variant="outline" onClick={handleDeleteSample} disabled={isProcessing} className="w-full sm:w-auto border-red-200 text-red-600 hover:bg-red-50"><Trash2 className="h-4 w-4 mr-2" />削除</Button>
                                <div className="flex gap-2 w-full sm:w-auto ml-auto">
                                    <Button variant="ghost" onClick={() => setEditingSample(null)} className="flex-1 sm:flex-none">キャンセル</Button>
                                    <Button onClick={handleSaveUsage} disabled={isProcessing || editUsedQty === ""} className="flex-1 sm:flex-none bg-blue-600 hover:bg-blue-700 text-white font-bold">{isProcessing ? <Loader2 className="animate-spin h-4 w-4 mr-2" /> : <Save className="h-4 w-4 mr-2" />} 保存</Button>
                                </div>
                            </DialogFooter>
                        </div>
                    )}
