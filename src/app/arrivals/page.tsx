...
// ===== 日付ユーティリティ =====
// Supabase が date / timestamp / timestamptz のどれで返してきても
// 先頭10文字の YYYY-MM-DD を優先して比較する
...
export default function ArrivalsPage() {
    ...
    const fetchData = useCallback(async () => {
        setLoading(true);
        try {
            const [itemsRes, arrivalsRes] = await Promise.all([
                supabase
                    .from("items")
                    .select("*")
            ...
            if (itemsRes.error) {
                ...
                alert("品目データの取得に失敗しました。");
            }
            if (arrivalsRes.error) {
                console.error("arrivals fetch error:", arrivalsRes.error);
                alert("入荷データの取得に失敗しました。");
            }
                ...
                <div className="w-full lg:w-[35%]">
                    {canEdit ? (
                        ...
                        <Card className="border-slate-200 bg-slate-50 opacity-70">
                            <CardContent className="p-8 md:p-16 text-center text-slate-500">
                                        ...
                                        <TableRow key={arrival.id} className="hover:bg-slate-50">
                                            ...
                                            <TableCell className="text-center">
                                                <Button
                                                    variant="outline"
                                                    size="sm"
                                                    onClick={() => openEditDialog(arrival)}
