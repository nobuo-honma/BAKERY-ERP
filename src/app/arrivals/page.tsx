                    ...
                    {day && (
                      <>
                        ...
                        <div className="space-y-1.5 print:space-y-1">
                          {dayArrivals.map(arr => (
                            <div key={arr.id} onClick={() => canEdit && openEditDialog(arr)} className={`${arr.status === 'arrived' ? "bg-green-50 border-green-300" : "bg-blue-50 border-blue-200"} border rounded p-1.5 print:p-1 text-xs leading-tight wrap-break-word relative group ${canEdit ? 'cursor-pointer hover:shadow-md transition-shadow' : ''}`}>
                    ...
                    {dayArrivals.map(arr => (
                      <div key={arr.id} onClick={() => canEdit && openEditDialog(arr)} className={`${arr.status === 'arrived' ? "bg-green-50 border-green-300" : "bg-blue-50 border-blue-200"} border rounded p-2.5 text-xs shadow-sm relative group ${canEdit ? 'cursor-pointer hover:bg-slate-50' : ''}`}>
                        <div className="flex justify-between items-start mb-1.5 border-b border-white/40 pb-1.5">
                          ...
                          {arr.status === 'arrived' ?
                            <span className="text-[10px] bg-green-600 text-white px-1.5 rounded font-bold flex items-center gap-0.5"><CheckCircle2 className="h-2.5 w-2.5" /> 入荷済</span> :
                            <span className="text-[10px] bg-blue-500 text-white px-1.5 rounded font-bold flex items-center gap-0.5"><PackageCheck className="h-2.5 w-2.5" /> 発注済</span>
                          }
                        </div>
        ...
        <div className="w-full lg:w-[35%]">
          {canEdit ? (
            <Card className="border-slate-200 shadow-sm sticky top-24">
              <CardHeader className="bg-slate-50 pb-4 border-b rounded-t-lg"><CardTitle className="text-lg flex items-center gap-2 text-slate-800"><Plus className="h-5 w-5" /> 新規入荷予定の登録</CardTitle></CardHeader>
              <CardContent className="pt-6 space-y-4">
                <div><label className="block text-sm font-bold mb-1 text-slate-700">対象品目 (原料・資材)</label><select value={newItemId} onChange={e => setNewItemId(e.target.value)} className="w-full border border-blue-200 rounded-md p-2.5 text-sm bg-white focus:ring-blue-500"><option value="">品目を選択</option><optgroup label="原材料">{items.filter(i => i.item_type === 'raw_material').map(i => <option key={i.id} value={i.id}>{i.name}</option>)}</optgroup><optgroup label="資材">{items.filter(i => i.item_type === 'material').map(i => <option key={i.id} value={i.id}>{i.name}</option>)}</optgroup></select></div>
                <div className="grid grid-cols-2 gap-4"><div><label className="block text-sm font-bold mb-1 text-slate-700">発注日</label><Input type="date" value={newOrderDate} onChange={e => setNewOrderDate(e.target.value)} className="bg-white" /></div><div><label className="block text-sm font-bold mb-1 text-blue-800">入荷予定日</label><Input type="date" value={newExpectedDate} onChange={e => setNewExpectedDate(e.target.value)} className="bg-white border-blue-300 shadow-sm" /></div></div>
                <div><label className="block text-sm font-bold mb-1 text-slate-700">発注・入荷予定数</label><div className="flex items-center gap-3"><Input type="number" min="0" value={newQuantity} onChange={e => setNewQuantity(e.target.value === "" ? "" : Number(e.target.value))} className="text-xl font-bold text-right border-blue-300 shadow-sm h-12" /><span className="text-lg font-bold text-slate-500 w-12">{selectedItemUnit || "-"}</span></div></div>
                <div><label className="block text-sm font-bold mb-1 text-slate-700">備考 (任意)</label><textarea value={newNotes} onChange={e => setNewNotes(e.target.value)} placeholder="発注先など..." className="w-full p-2 border border-slate-300 rounded-md text-sm resize-none h-20" /></div>
